...
namespace MapGenerator
{
  ...
  class LandGenerator : Generator
  {
    ...
    protected override void LoadContent( )
    {
      ...
      map_ground = new DungenMap( Global_Settings.draw_width / Global_Settings.tile_pixels, Global_Settings.draw_height / Global_Settings.tile_pixels );
    ...
    public void generate_ground_map( )
    {
      ...
      for ( int i = 0; i < map_ground.Width; i++ )
      {
        for ( int j = 0; j < map_ground.Height; j++ )
        {
          Console.WriteLine( " [" + i + ", " + j + "] " + map_ground.tiles[ i, j ].type.ToString( ) );
        }
      }
    }
    ...
    public void render_ground_map( )
    {
      ...
      for ( int biom = max_bioms; biom >= 0; biom-- )
      {
        for ( int i = 1; i < map_ground.Width; i++ )
        {
          ...
          {
            if ( (int)map_ground.tiles[ i, j ].type == biom )
            {

              if ( (int)map_ground.tiles[ i - 1, j - 1 ].type == biom )
              {
                if ( (int)map_ground.tiles[ i, j - 1 ].type != biom && (int)map_ground.tiles[ i - 1, j ].type != biom )
                {

                  Console.WriteLine( "Changed Tile [" + i + ", " + ( j - 1 ) + "].type " + map_ground.tiles[ i, j - 1 ].type + " to " + ( (TILE_TYPE)biom ).ToString( ) +
                                     " because Tile [" + i + ", " + j + "].type " + map_ground.tiles[ i, j ].type );
                  //map[i - 1, j - 1].type = map[i, j - 1].type;
                  map_ground.tiles[ i, j - 1 ].change_biom( (TILE_TYPE)biom );
                }
              }

              if ( i < map_ground.Height - 1 && (int)map_ground.tiles[ i + 1, j - 1 ].type == biom )
              {
                if ( (int)map_ground.tiles[ i, j - 1 ].type != biom && (int)map_ground.tiles[ i + 1, j ].type != biom )
                {
